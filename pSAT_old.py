...
class Solver:
    def __init__(self, problem, integrator) -> None:
        ...
        self.w_ = np.identity(self.problem.number_of_variables)
        self.e_ = np.zeros(self.problem.number_of_variables)
    ...
    def save_states(self) -> None:
        ...
        self.es.append(self.e_/self.time)
        #self.es.append(self.e_)
    ...
    def step_function(self, adaptive_flag, tangental_flag, lower_diff_bound, upper_diff_bound, lower_h_bound, upper_h_bound) -> None:
        ...
        if tangental_flag:
            J_ = ( np.identity(self.problem.number_of_variables) + self.problem.Jakobian(self.s) )*self.integrator.h
            self.w_ = orth(J_ @ self.w_)
            for i in range(self.problem.number_of_variables):
                self.e_[i] += np.log(np.linalg.norm(self.w_[:,i]))
                #self.e_[i] += np.log(np.linalg.norm(self.w_[:,i]))/self.integrator.h
                self.w_[:,i] = self.w_[:,i]/np.linalg.norm(self.w_[:,i])
    ...
    def solve(self, time_limit = 2.0, adaptive_flag = True, tangental_flag = True, save_frequency = 2, lower_diff_bound = 0.00005, upper_diff_bound = 0.01, lower_h_bound = 0.00005, upper_h_bound = 0.01) -> None:
